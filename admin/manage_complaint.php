<?php include 'header_adminpages.php';
    ...
    // $result = mysqli_query($conn,"SELECT * FROM customer_feedback WHERE feedback_id= '$a' AND message_type='complaint'");
    $result = mysqli_query($conn,"SELECT * FROM customer_feedback WHERE feedback_id= '$a'");
    $row = mysqli_fetch_array($result);
?>

<h2 class="text-center">Manage Complaint</h2>
<div class="update-item bg-light">
  <form name= "form_update_item" method="post" action="">
    ...
    <div class="row">
      ...
      <div class="col-md-3">
        <button type="submit" class="btn btn-primary btn-seperate btn-sm" name="updatecomplaint">Update your Information</button>
      </div>
      <div class="col-md-3">
        <button type="submit" class="btn btn-primary btn-seperate btn-sm" name="deletecomplaint">Delete your Information</button>
      </div>
      <div class="col-md-3">        
      </div>      
    </div>
...
if (isset($_POST['updatecomplaint']))
{
    ...
    if($query)
    {
        echo "<h2>Your complaint is updated Successfully</h2>";
